package com.js.interpreter.gui;
...
public class IDE extends JFrame {
	...
	static String pluginsPackage = "com.js.interpreter.plugins";
	...
	public IDE() {
		...
		this.addNewPluginsDirectory(new File(System.getProperty("user.dir")
				+ File.separatorChar + "bin" + File.separatorChar + "com"
				+ File.separatorChar + "js" + File.separatorChar
				+ "interpreter" + File.separatorChar + "plugins"
				+ File.separatorChar));
		this.type_generator = new CustomTypeGenerator(new File(
				System.getProperty("user.dir") + File.separatorChar + "bin"));
		...
		if (ProgramThread != null && ProgramThread.isAlive()) {
			...
			try {
				...
				for (PluginDeclaration p : plugins) {
					functionTable.put(p.name(), p);
				}
				program = new PascalProgram(new StringReader(
						programInput.getText()), functionTable, "IDE_Textbox",
						new ArrayList<ScriptSource>() {
							{
								add(new FileScriptSource(System
										.getProperty("user.dir")));
							}
						}, type_generator);
			} catch (ParsingException e) {
				debugBox.append(e.line
						+ ": The following error trying to parse this program: "
						+ e.getMessage() + '\n');
			}

			ProgramThread = new Thread(program.run());
			ProgramThread.start();
		}
	}
